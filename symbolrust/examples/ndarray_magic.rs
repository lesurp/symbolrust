...
fn main() {
    ...
    let d = c.map(|node| Evaluator::evaluate(node, &variables));
    
    println!("{}", PrettyPrinter::print_array_with_context(&c, &pp_context));
    pp_context.name_var(x, "x");
    println!("{}", PrettyPrinter::print_array_with_context(&c, &pp_context));
    println!("{}", PrettyPrinter::print_array_with_context(&d, &pp_context));
}
